...
export function ChatPanel({ agent, onNavigateHistory, onNavigateSettings, onOpenCanvas, onToast }: ChatPanelProps) {
  ...
  return <div className="chat-panel">
    ...
    {tab==='context'&&<div className="chat-body chat-tab-body"><div className="chat-tab-heading"><b>{t('Agent context')}</b><span>{t('Local agent configuration')}</span></div><div className="context-block"><span>{t('Instructions')}</span><p>{agent.instructions}</p></div><div className="context-block"><span>{t('Workspace')}</span><p>{getWorkspaceById(agent.workspaceId)?.name ?? agent.workspaceId}</p></div><div className="context-block"><span>{t('Tools')}</span><div className="context-tools">{agent.tools.map((tool)=><i key={tool}>{agentToolDetails[tool].label}</i>)}</div></div>{agent.skillsDirectory&&<div className="context-block"><span>{t('Skills')}</span><p>{agent.skillsDirectory}</p><div className="context-tools">{(agent.skills??[]).map((skill)=><i key={skill}>{skill}</i>)}</div></div>}<div className="context-note"><Icon name="link"/>{t('Canvas context will include selected objects when Codex is connected.')}</div></div>}
