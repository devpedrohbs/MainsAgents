...
export function Home({tasks,onOpenTask}:{tasks:Task[];onOpenTask:(task:Task)=>void}) {
  ...
  return <div className="page mission-control"><div className="page-head"><div><p className="eyebrow">{formatToday(locale)}</p><h1>{t('Mission Control')}</h1><p>{t('See what is moving, where attention is needed, and what your agents delivered.')}</p></div></div><div className="mission-strip" aria-label="Workspace status"><div className="mission-stat"><i/><span><strong>{agents.filter((agent)=>agent.status==='working').length}</strong> {t('agents working')}</span></div><div className="mission-stat attention"><i/><span><strong>{agents.filter((agent)=>agent.status==='review').length}</strong> {t('needs your attention')}</span></div><div className="mission-stat done"><i/><span><strong>{completedTasks.length}</strong> {t('tasks completed today')}</span></div></div><section className="section"><div className="section-title"><h2>{t('Working now')}</h2><span>{workingTasks.length} {t('active tasks')}</span></div><div className="activity-list">{workingTasks.map(activityRow)}</div></section>{attentionTasks.length>0&&<section className="section attention-section"><div className="section-title"><h2>{t('Needs your attention')}</h2><span>{attentionTasks.length} {t(attentionTasks.length===1?'item':'items')}</span></div><div className="attention-list">{attentionTasks.map((task)=>{const agent=getAgentById(task.agentId)??fallback;return <div className="attention-row" key={task.id}><div className="attention-icon"><Icon name="spark"/></div><div className="attention-copy"><b>{task.title}</b><span>{agent?.name??'Agent'} · {task.metadata}</span></div><button className="soft-button" onClick={()=>onOpenTask(task)}>{t('Review')} <Icon name="chevron"/></button></div>})}</div></section>}<section className="section"><div className="section-title"><h2>{t('Recently completed')}</h2><span>{completedTasks.length} {t('delivered today')}</span></div><div className="activity-list">{completedTasks.map(activityRow)}</div></section></div>;
}
...
function formatToday(locale:string):string { return new Intl.DateTimeFormat(locale,{weekday:'long',month:'short',day:'numeric'}).format(new Date()).toUpperCase(); }
