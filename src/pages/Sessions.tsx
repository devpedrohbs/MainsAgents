...
export function Sessions({activeAgentId,onActivateAgent,onToast}:SessionsProps) {
  ...
  return <div className="page"><div className="page-head"><div><p className="eyebrow">Agent history</p><h1>Sessions</h1><p>Each conversation keeps its own messages, agent, and working context.</p></div><button className="primary-button" onClick={startSession}><Icon name="plus"/>New session</button></div><div className="sessions-list">{ordered.map((session)=>{const agent=getAgentById(session.agentId);return <button className="session-row" key={session.id} onClick={()=>open(session.agentId,session.id,session.title)}><span className="session-main"><b>{session.title}</b><span>{session.messages.length} items in this conversation</span></span><span className="session-agent">{agent?.name??'Deleted agent'}</span><span className="session-context">Independent session</span><span className="time">{formatUpdated(session.updatedAt)}</span></button>})}</div></div>;
}
